...
public class ShoppingCart {
    ...
    public void ShowMainShoppingMenu(Restaurant restaurant){
        ...
        while(laststagestatus != true){
            ...
            if(input == "1"){
                ...
                Input = input;
                ShowConfirmOrder(user,Orderlist,timeadd);
            }
            else if(input == "2"){
                laststagestatus = true;
                Input = input;
    ...
    public void AddCartItem(User user,Restaurant restaurant) {
        restaurant.ShowFoodMenu();
                ...
                int price = calculator.CalculateTotalprice(foodnumber,amount,pricechoice,this.restaurant);
    ...
    public void ShowConfirmOrder(User user,List<Order> Orderlist,PickUpTime timebook){
        Console.Clear();
        Console.WriteLine("Your Order Has been sent to the restaurant");
        int n =1;
        foreach(Order order in Orderlist){
            Console.WriteLine("{0} : {1} X {2} : {3} Baht",n,order.GetMenuName(),order.GetAmount(),order.GetTotalPrice());
            Console.WriteLine("---------------------------------");
            n++;
        }
        ...
        UserOrderHistory userorder = new UserOrderHistory(timebook);
        foreach(Order order in Orderlist){
